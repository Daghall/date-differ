...
function getDays(from, to) {
  ...
  if (fromDay > toDay) {
    const remainingDays = getRemainingDaysInMonth(from, to);
    return remainingDays + toDay;
  }
  return toDay - fromDay;
}

// Returns the remaining days of the month before target month
function getRemainingDaysInMonth(from, to) {
  const date = new Date(to);
  date.setMonth(to.getMonth() - 1);
  const currentTime = date.setDate(from.getDate());
  let remainingDays = 1;
